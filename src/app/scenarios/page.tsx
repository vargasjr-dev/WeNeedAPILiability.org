...
const hardcodedScenarios: ScenarioData[] = [
  ...
  },
  {
    ...
    description:
      "Multiple AI agents work together to accomplish complex tasks, with each agent making decisions that affect the others and the overall outcome.",
    today:
...
async function getAllScenarios(): Promise<ScenarioData[]> {
  try {
    ...
    const filteredHardcoded = hardcodedScenarios.filter(
      (s) => !dbScenarioSlugs.has(s.slug),
    );
    return [
      ...filteredHardcoded,
      ...dbScenarios.map((s) => ({
